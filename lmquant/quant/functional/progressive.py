...
def progressive_quantize(  # noqa: C901
    ...
    *,
    ...
    # develop_tensor = develop_tensor.view(compute_view_shape).div(compute_scale.data).view(shape)
    # develop_tensor = develop_tensor.clamp(min=compute_quant_range.min, max=compute_quant_range.max)
    result = onestep_quantize(
        develop_tensor,
    ...
    if return_with_dequant:
        ...
        if config.saturate_compute_dtype:
            ...
            # tensor_hat = tensor_hat.clamp(min=config.compute_dtype.max_value, max=config.compute_dtype.min_value)
        else:
        ...
        result._dequantized = tensor_hat.view(compute_view_shape)
        result._dequantized = result._dequantized.mul(compute_scale.data).view(shape).to(dtype)
