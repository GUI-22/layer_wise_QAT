...
def run(  # noqa: C901
    ...
    needs_reorder = quant and config.quant.enabled_reorder
    needs_smooth = quant and config.quant.enabled_smooth
    ...
    if needs_rotation:
        ...
        if config.enable_cache and config.cache_path.rotation and os.path.exists(config.cache_path.rotation):
            ...
            if config.enable_cache and config.cache_path.rotation:
    ...
    hooks_for_copied_model: dict[str, dict[str, list[torch.utils.hooks.RemovableHandle]]] = dict(reorder={}, activation={})
    # region reorder channels
    if needs_reorder:
        logger.info("* Reordering channels")
        tools.logging.Formatter.indent_inc()
        if config.enable_cache and config.cache_path.reorder and os.path.exists(config.cache_path.reorder):
            ...
            _, reorder_hooks_for_copied_model = reorder_llm(
                model_copied, config.quant, tokenizer, calib_config=config.calib, reorder_cache=reorder_cache
            )
        else:
            ...
            _, reorder_hooks_for_copied_model = reorder_llm(model_copied, config.quant, tokenizer, calib_config=config.calib)
            if config.enable_cache and config.cache_path.reorder:
        ...
        hooks_for_copied_model["reorder"] = reorder_hooks_for_copied_model
        del reorder_cache, reorder_hooks, reorder_hooks_for_copied_model
        tools.logging.Formatter.indent_dec()
        gc.collect()
        torch.cuda.empty_cache()
    # endregion
    ...
    if needs_smooth:
        ...
        tools.logging.Formatter.indent_inc()
        if config.enable_cache and config.cache_path.smooth and os.path.exists(config.cache_path.smooth):
            ...
            if config.enable_cache and config.cache_path.smooth:
        ...
        torch.cuda.empty_cache()
    with torch.no_grad():
        for param, param_copied in zip(model.module.parameters(), model_copied.module.parameters()):
            param_copied.copy_(param)
    # endregion
    ...
    # # region copied_model rotate
    # if needs_rotation:
    #     logger.info(f"* Building model_copied {config.model.name} from {config.model.path}")
    #     model_copied, tokenizer = config.model.build(dtype=torch.float32, cpu=config.model.size > 30)
    #     model_copied = LlmModelStruct.build(model_copied)
    #     config.quant.num_hidden_layers = model_copied.config.num_hidden_layers
    #     if config.quant.develop_dtype is None:
    #         config.quant.develop_dtype = torch.float64
    #     logger.info(f"* Development dtype is {config.quant.develop_dtype}")
    #     logger.info("* Rotating model_copied")
    #     tools.logging.Formatter.indent_inc()
    #     if config.enable_cache and config.cache_path.rotation and os.path.exists(config.cache_path.rotation):
    #         logger.info(f"- Loading rotation from {config.cache_path.rotation}")
    #         rotation = torch.load(config.cache_path.rotation)
    #         rotate_llm(model_copied, config.quant.rotation, rotation=rotation)
    #     else:
    #         logger.info("- Generating rotation")
    #         rotation = rotate_llm(model_copied, config.quant.rotation)
    #         if config.enable_cache and config.cache_path.rotation:
    #             logger.info(f"- Saving rotation to {config.cache_path.rotation}")
    #             os.makedirs(config.cache_dirpath.rotation, exist_ok=True)
    #             torch.save(rotation, config.cache_path.rotation)
    ...
    #     gc.collect()
    #     torch.cuda.empty_cache()
    #     rotated_state_dict_for_copied = {name: param.data.cpu() for name, param in model_copied.module.named_parameters()}
    #     del model_copied, tokenizer
    #     gc.collect()
    #     torch.cuda.empty_cache()
    #     model_copied, tokenizer = config.model.build()
    #     model_copied = LlmModelStruct.build(model_copied)
    #     logger.info("* Rotating model_copied in float16 again")
    #     tools.logging.Formatter.indent_inc()
    #     rotate_llm(model_copied, config.quant.rotation, rotation=rotation)
    #     for name, param in model_copied.module.named_parameters():
    #         if name in rotated_state_dict_for_copied:
    #             param.data = rotated_state_dict_for_copied[name].data.to(dtype=param.dtype, device=param.device)
    #     del rotated_state_dict_for_copied, rotation
    #     tools.logging.Formatter.indent_dec()
    #     gc.collect()
    #     torch.cuda.empty_cache()
    # else:
    #     logger.info(f"* Building model_copied {config.model.name} from {config.model.path}")
    #     model_copied, tokenizer = config.model.build(dtype=torch.float16)
    #     model_copied = LlmModelStruct.build(model_copied)
    #     config.quant.num_hidden_layers = model_copied.config.num_hidden_layers
    #     if config.quant.develop_dtype is None:
    #         config.quant.develop_dtype = torch.float32
    ...
    # hooks_for_copied: dict[str, dict[str, list[torch.utils.hooks.RemovableHandle]]] = dict(reorder_for_copied={}, activation_for_copied={})
    # # region copied_model reorder channels
    # if needs_reorder:
    #     logger.info("* Reordering channels")
    #     tools.logging.Formatter.indent_inc()
    #     if config.enable_cache and config.cache_path.reorder and os.path.exists(config.cache_path.reorder):
    #         logger.info(f"- Loading channel indices from {config.cache_path.reorder}")
    #         reorder_cache = torch.load(config.cache_path.reorder)
    #         _, reorder_hooks_for_copied = reorder_llm(
    #             model_copied, config.quant, tokenizer, calib_config=config.calib, reorder_cache=reorder_cache
    #         )
    #     else:
    #         logger.info("- Generating channel indices")
    #         reorder_cache, reorder_hooks_for_copied = reorder_llm(model_copied, config.quant, tokenizer, calib_config=config.calib)
    #         if config.enable_cache and config.cache_path.reorder:
    #             logger.info(f"- Saving channel indices to {config.cache_path.reorder}")
    #             os.makedirs(config.cache_dirpath.reorder, exist_ok=True)
    #             torch.save(reorder_cache, config.cache_path.reorder)
    #     hooks["reorder_for_copied"] = reorder_hooks_for_copied
    #     del reorder_cache, reorder_hooks_for_copied
    #     tools.logging.Formatter.indent_dec()
    #     gc.collect()
    #     torch.cuda.empty_cache()
    # # endregion
    ...
    # # region copied_model smooth quantization
    # if needs_smooth:
    #     logger.info("* Smooth quantizing model_copied")
    #     tools.logging.Formatter.indent_inc()
    #     if config.enable_cache and config.cache_path.smooth and os.path.exists(config.cache_path.smooth):
    #         logger.info(f"- Loading smooth scales from {config.cache_path.smooth}")
    #         smooth_llm(model_copied, config.quant, smooth_cache=torch.load(config.cache_path.smooth))
    #     else:
    #         logger.info("- Generating smooth scales")
    #         smooth_cache = smooth_llm(model_copied, config.quant, tokenizer=tokenizer, calib_config=config.calib)
    #         if config.enable_cache and config.cache_path.smooth:
    #             logger.info(f"- Saving smooth scales to {config.cache_path.smooth}")
    #             os.makedirs(config.cache_dirpath.smooth, exist_ok=True)
    #             torch.save(smooth_cache, config.cache_path.smooth)
    ...
    if needs_qat:
        ...
        qat_llm(
            ...
            seq_length=1024,
            num_train_rows=128,
            num_train_tokens=128 * 1024,
            batch_size=32,
            cache_save_path="/data/gyy/lmquant-main/lmquant/llm/kd_data/",
            lr=1e-2,
            num_epochs=5,
            weight_dacay=1e-2,
            kl_temperature=2.0,
            ...
            loss_scale=1e-1,
            eps=1e-4,
            fairseq_args=config.fairseq_args,
            gen_teacher_opts=config.gen_teacher_opts,
            orig_model_path=config.model.path,
            orig_model=model_copied
        )
    # endregion
    ...
    if quant_wgts:
        ...
        tools.logging.Formatter.indent_inc()
        if config.enable_cache and config.cache_path.wgts and os.path.exists(config.cache_path.wgts):
            ...
            if config.enable_cache and config.cache_dirpath.wgts:
    ...
    if quant_acts:
        ...
        tools.logging.Formatter.indent_inc()
        if config.enable_cache and config.cache_path.acts and os.path.exists(config.cache_path.acts):
            ...
            if config.enable_cache and config.cache_dirpath.acts and quant_cache is not None:
