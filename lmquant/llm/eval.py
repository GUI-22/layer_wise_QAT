...
class LlmEvalConfig(BaseEvaluationConfig):
    ...
    def evaluate(
        self, model: AutoModelForCausalLM, /, tokenizer: AutoTokenizer, model_name: str, **kwargs
        ...
        """
        if "val_valid" not in self.tasks:
            self.tasks.append("val_valid")
                ...
                for task in self.tasks:
                    rst["results"][task] = {
                        "word_perplexity": _eval_ppl_with_gptq_evaluator(
                            model, tokenizer, task=task, seq_length=max_seq_length, **kwargs
                        )
                    }
                    rst["versions"][task] = 1
...
def _eval_ppl_with_gptq_evaluator(
    ...
    **kwargs
    ...
    assert "data_path" in kwargs
    if task.startswith("wikitext"):

        # test_dataset = load_dataset("wikitext", "wikitext-2-raw-v1", split="test")
        # with open("wikitext2_raw_v1.json", "w") as file:
        #     json.dump(test_dataset["text"], file)

        with open("wikitext2_raw_v1.json", "r") as file:
            raw_data = json.load(file)
        test_dataset = tokenizer("\n\n".join(raw_data), return_tensors="pt")

    elif task.startswith("pile"):
        test_dataset = load_dataset("pile", task, split="test")
        test_dataset = tokenizer("\n\n".join(test_dataset["text"]), return_tensors="pt")
    elif task.startswith("val_valid"):
        raw_data = []
        with open(kwargs["data_path"], "r") as file:
            for line in file:
