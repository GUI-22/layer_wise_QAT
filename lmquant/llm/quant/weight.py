...
def quantize_llm_decoder_layer_weights(  # noqa: C901
...
) -> tuple[dict[str, nn.Parameter], dict[str, WeightQuantizer], dict[str, torch.Tensor | float | None]]:
    ...
    Returns:
        tuple[dict[str, nn.Parameter], dict[str, WeightQuantizer], dict[str, torch.Tensor | float | None]: Original weights, Quantizers, and scale state dict.
    """
    ...
    original_weights: dict[str, nn.Parameter] = {}
    for key, module, module_name, eval_module, eval_name, eval_kwargs in args_caches:
        quantizer_config = config.specialize_for(key, layer_idx=layer.idx).wgts
        quantizer = WeightQuantizer(quantizer_config, develop_dtype=config.develop_dtype, key=key)
        ...
        if quantizer.enabled:
            ...
            original_weights[module_name] = copy.deepcopy(module.weight)
    ...
    return quantizers, scale_state_dict, original_weights
...
def quantize_llm_weights(
    ...
    dict[int, dict[str, nn.Parameter]]
]:
    ...
    quantizers: dict[str, WeightQuantizer] = {}
    scale_state_dict: dict[str, torch.Tensor | float | None] = {}
    layers_original_weights: dict[int, dict[str, nn.Parameter]] = {}
    if not quant_cache or quant_config.wgts.enabled_calib_kernel:
        with logging_redirect_tqdm():
            for _, (layer, layer_cache, layer_kwargs) in tqdm(
                ...
                if not (layer.idx >= begin_layer_idx and (end_layer_idx is None or layer.idx < end_layer_idx)):
                    continue

                block_quantizers, block_state_dict, layer_original_weights = quantize_llm_decoder_layer_weights(
                    layer=layer,
                    config=quant_config,
                ...
                layers_original_weights[layer.idx] = layer_original_weights
    ...
    return quant_cache, quantizers, scale_state_dict, layers_original_weights


def quantize_llm_decoder_layer_weights_with_grad(  # noqa: C901
    layer: LlmDecoderLayerStruct,
    config: LlmQuantConfig,
    quant_cache: dict[str, dict[str, torch.Tensor | float | None]],
    layer_cache: dict[str, IOActivationsCache] | None = None,
    ...
    """
    logger = logging.getLogger(f"{__name__}.WeightQuant")
    logger.info("- Quantizing decoder layer %s", layer.full_name)
    tools.logging.Formatter.indent_inc()
    layer_cache = layer_cache or {}
    layer_kwargs = layer_kwargs or {}
    ...
    with torch.inference_mode():
        for key, module, module_name, eval_module, eval_name, eval_kwargs in args_caches:
            ...
            if quantizer.enabled:
                if module_name not in quant_cache:
                    logger.info("- Calibrating %s.weight", module_name)
                    quantizer.calibrate_dynamic_range(
                        module=module,
                        inputs=layer_cache.get(module_name, IOActivationsCache()).inputs,
                        eval_inputs=layer_cache.get(eval_name, IOActivationsCache()).inputs,
    ...
    module_weights: dict[str, nn.Parameter] = {}
    ...
    for key, module, module_name, eval_module, eval_name, eval_kwargs in args_caches:
        ...
        param_name = f"{module_name}.weight"
        if quantizer.enabled:
            logger.info("- Quantizing %s", param_name)
            module_weights[module_name] = module.weight
            quantizer.load_state_dict(quant_cache[module_name], device=module.weight.device)
            ...
            dequantized_weights[module_name] = result.data
            module.weight = nn.Parameter(result.data)
    ...
    del args_caches
    tools.logging.Formatter.indent_dec()
    return module_weights, dequantized_weights


def quantize_llm_weights_with_grad(
    model: nn.Module | LlmModelStruct,
    quant_config: LlmQuantConfig,
    tokenizer: nn.Module | None = None,
    calib_config: LlmCalibConfig | None = None,
