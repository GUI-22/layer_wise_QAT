...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, criterion_valid=None, quantizer=None):
        ...
        if criterion_valid is None:
            self._criterion_valid = self._criterion
        else:
            self._criterion_valid = criterion_valid
            if not self.is_fsdp:
                if cfg.common.fp16:
                    self._criterion_valid = self._criterion_valid.half()
                elif cfg.common.bf16:
                    self._criterion_valid = self._criterion_valid.to(dtype=torch.bfloat16)
            if (
                not cfg.distributed_training.pipeline_model_parallel
                # the DistributedFairseqModel wrapper will handle moving to device,
                # so only handle cases which don't use the wrapper
                ...
                self._criterion_valid = self._criterion_valid.to(device=self.device)
        ...
        self._wrapped_criterion_valid = None
    ...
    @property
    def criterion_valid(self):
        if self._wrapped_criterion_valid is None:
            if (
                utils.has_parameters(self._criterion_valid)
                and self.use_distributed_wrapper
            ):
                self._wrapped_criterion_valid = models.DistributedFairseqModel(
                    self.cfg.distributed_training,
                    self._criterion_valid,
                    process_group=self.data_parallel_process_group,
                    device=self.device,
                )
            else:
                self._wrapped_criterion_valid = self._criterion_valid
        return self._wrapped_criterion_valid
    ...
    def state_dict(self, filename, training_finished=False) -> Dict[str, Dict]:
        ...
        for filename, model_state_dict, optimizer_state_dict in model_save_list:
            state_dict = {
                ...
                "criterion_valid": (
                    self.criterion_valid.state_dict()
                    if utils.has_parameters(self.criterion_valid) else None
                ),
                "optimizer_history": (self._optim_history or [])
                + [
                    {
                        "criterion_name": self.get_criterion().__class__.__name__,
                        "criterion_valid_name": self.get_criterion_valid().__class__.__name__,
        ...
        if bexists:
            ...
            try:
                ...
                if utils.has_parameters(self.get_criterion()):
                    ...
                    del state["criterion"]
                if utils.has_parameters(self.get_criterion_valid()):
                    self.get_criterion_valid().load_state_dict(
                        state["criterion_valid"], strict=True
                    )
                    del state["criterion_valid"]
    ...
    def get_valid_ppl_iterator(
        ...
        """Return an EpochBatchIterator over the training set for a given epoch."""
        if load_dataset:
            logger.info("loading valid data for epoch {}".format(epoch))
            self.task.load_dataset(
                self.cfg.dataset.valid_subset,
                epoch=epoch,
                combine=combine,
                data_selector=data_selector,
            )
        batch_iterator = self.task.get_batch_iterator(
            dataset=self.task.dataset(self.cfg.dataset.valid_subset),
            max_tokens=self.cfg.dataset.max_tokens,
            max_sentences=self.cfg.dataset.batch_size,
            max_positions=utils.resolve_max_positions(
                self.task.max_positions(),
    ...
    def get_teacher_input_text_iterator(
        ...
        batch_iterator = self.task.get_teacher_input_text_iterator(
            dataset=self.task.dataset(self.cfg.dataset.train_subset),
            max_tokens=self.cfg.dataset.max_tokens,
            max_sentences=self.cfg.dataset.batch_size,
            max_positions=utils.resolve_max_positions(
    ...
    def get_teacher_args_and_outputs_iterator(
        ...
        batch_iterator = self.task.get_teacher_args_and_outputs_iterator(
            dataset=self.task.dataset(self.cfg.dataset.train_subset),
            max_tokens=self.cfg.dataset.max_tokens,
            max_sentences=self.cfg.dataset.batch_size,
            max_sentences_training=self.cfg.dataset.batch_size,
            max_positions=utils.resolve_max_positions(
                self.task.max_positions(),
                self.model.max_positions(),
                self.cfg.dataset.max_tokens,
    ...
    def get_layer_kwargs(
        ...
        if hasattr(self.task, "get_layer_kwargs"):
            kwargs_list = self.task.get_layer_kwargs(
                dataset=self.task.dataset(self.cfg.dataset.train_subset)
                )
        else:
            kwargs_list = None
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        with torch.no_grad():
            ...
            try:
                ...
                _loss, sample_size, logging_output = self.task.valid_step(
                    sample, self.model, self.criterion_valid
                )
            except RuntimeError as e:
                if "out of memory" in str(e):
                    self._log_oom(e)
        ...
        # gather logging outputs from all replicas
        if self.data_parallel_world_size > 1:
            logging_outputs, (sample_size,) = self._aggregate_logging_outputs_valid(
                logging_outputs,
                sample_size,
        ...
        logging_output = self._reduce_and_log_stats_valid(logging_outputs, sample_size)
    ...
    def get_criterion_valid(self):
        """Get the (non-wrapped) criterion_valid instance."""
        return self._criterion_valid
    ...
    def _aggregate_logging_outputs_valid(
        self,
        logging_outputs: List[Dict[str, Any]],
        *extra_stats_to_sum,
        ignore=False,
    ):
        if self.task.__class__.logging_outputs_can_be_summed(self.get_criterion_valid()):
            return self._fast_stat_sync_sum(
                logging_outputs, *extra_stats_to_sum, ignore=ignore
            )
        else:
    ...
    def _reduce_and_log_stats_valid(self, logging_outputs, sample_size, grad_norm=None):
        if grad_norm is not None and (
            not torch.is_tensor(grad_norm) or torch.isfinite(grad_norm)
        ):
            metrics.log_speed("ups", 1.0, priority=100, round=2)
        ...
        with metrics.aggregate() as agg:
            if logging_outputs is not None:
                self.task.reduce_metrics(logging_outputs, self.get_criterion_valid())
                del logging_outputs

            # extra warning for criterions that don't properly log a loss value
            if "loss_vallid" not in agg:
                if "loss_valid" not in self._warn_once:
                    self._warn_once.add("loss_valid")
                    logger.warning(
                        "Criterion.reduce_metrics did not log a 'loss_valid' value, "
                        "which may break some functionality"
                    )
                metrics.log_scalar("loss_valid", -1)
